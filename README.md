...
- **Responsive** — phone portrait is the base layout; from 900px the top-level pages get a left sidebar, a Books list on the path, a two-pane Lexicon and a two-column reader, and session screens become a framed centered column. Short landscape phones get tighter spacing.
...
**Phase 6 — Learner feedback (planned, in order)**
1. [x] **Dark-mode flash on refresh.** Apply the saved theme/mode synchronously in `<head>` before any stylesheet loads (tiny inline script reading `localStorage`), give `<html>` a themed background, add `<meta name="color-scheme">`, and update `theme-color` per mode.
2. [x] **Locked means locked.** Lessons open only in order; the next lesson unlocks when the previous one is completed. Escape hatches: the placement test, and a Settings switch "Unlock all lessons" for learners who know what they are doing.
3. [x] **Resume a lesson.** Persist the live session (queue, position, answers so far, XP earned) per lesson/mode in the store; reopening offers "Resume" or "Start over"; abandoned sessions expire after 24 hours. Covers drill, alphabet, grammar, reading, placement.
4. [x] **Lexicon → Read goes to the word.** Build a per-lemma occurrence index (first occurrences by canonical order plus a total), list them in the Lexicon detail, and make "Read" open the first occurrence in the reader with the word highlighted.
5. [x] **Desktop layout for lesson screens.** The session screens (drill, alphabet, grammar, reading, placement) still use the phone layout on a wide window: a narrow framed column with the Continue button pinned to the bottom of an otherwise empty viewport. Redesign for ≥900px: a wider working column (~900px), the action button placed directly under the content instead of fixed to the window edge, a compact sticky header, vertical centering for short steps, and two-column steps where the content earns it — verse beside its gloss panel in Read & Translate, paradigm table beside the question in grammar, keyboard beside the answer field. Keyboard hints (1–4, Enter) shown on desktop. Phone layout unchanged.
6. [ ] **Dev testing mode.** A hidden mode for exercising the app without grinding through it, unlocked from Settings with a simple password ("Logos"). Not security — the site is static and the check is client-side — just a gate so casual users don't stumble into it. When on: every lesson unlocked regardless of item 2; a "Jump to lesson" picker; buttons to seed a sample learner (words, endings, verses, streak, XP) and to make everything due right now; skip to the summary of any session; time-travel the clock (advance a day) to test spaced repetition; show raw records for the current item; and a visible "DEV" badge so it is never mistaken for real progress. Turning it off restores normal behaviour; a reset returns the store to a clean state.
7. [ ] **Installed-app polish (iPhone + Mac).** Standalone display mode: safe-area padding, `100dvh` layouts, no rubber-band overscroll, no double-tap zoom on controls, status-bar style per theme, a proper splash/launch background, external links opening outside the app, Mac "Add to Dock" checked in Safari.
8. [ ] **Writing assignments.** A "Write & translate" step at the end of reading lessons: copy the verse by hand (paper, or on-screen with the tracing canvas), then type your own translation and compare against the reference; self-rated, XP-rewarded, recorded in the reading history.
9. [ ] **Handwriting tracing.** A canvas in the alphabet lessons: trace each letter over a faint guide with finger, pencil or mouse; stroke coverage check; re-usable by the writing assignments.
